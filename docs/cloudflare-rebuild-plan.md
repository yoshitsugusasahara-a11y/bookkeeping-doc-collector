...
**引き継ぎの取り決めは [cloudflare-carryover.md](cloudflare-carryover.md) に移した。そちらが正。**
2026-09-10に、下記の「gitの範囲指定」だけでは足りないことが判明したため（作り直す領域に入れた
判断は、持ち込み対象ファイルを1つも変更しないので範囲指定に現れない）。持ち込み対象の一覧、
差分の出し方、渡す時期、そして**判断と教訓の記録**はすべてそちらにある。
